...
void ScreenManager::AddDisplayController(
  ...
  VLOG(1) << "ScreenManager::AddDisplayController crtc=" << crtc
          << " connector=" << connector
          << " tile=" << (tile_property.has_value() ? "yes" : "no")
          << " controller_count_before=" << controllers_.size();
  ...
  VLOG(1) << "ScreenManager::AddDisplayController controller_count_after="
          << controllers_.size();
}

void ScreenManager::AddDisplayControllersForDisplay(const DrmDisplay& display) {
  VLOG(1) << "ScreenManager::AddDisplayControllersForDisplay display_id="
          << display.display_id()
          << " primary_crtc=" << display.GetPrimaryCrtcId()
          << " primary_connector=" << display.GetPrimaryConnectorId();
...
void ScreenManager::AddWindow(gfx::AcceleratedWidget widget,
  ...
  VLOG(1) << "ScreenManager::AddWindow widget=" << widget
          << " initial_bounds=" << window->bounds().ToString()
          << " controller_count=" << controllers_.size();
...
void ScreenManager::UpdateControllerToWindowMapping() {
  ...
  VLOG(1) << "ScreenManager::UpdateControllerToWindowMapping begin controllers="
          << controllers_.size() << " windows=" << window_map_.size();
  ...
  for (const auto& controller : controllers_) {
    const gfx::Rect controller_bounds(controller->origin(),
                                      controller->GetModeSize());
    VLOG(1) << "  controller candidate bounds=" << controller_bounds.ToString()
            << " enabled=" << controller->IsEnabled()
            << " has_master=" << controller->GetDrmDevice()->has_master();
    if (!controller->IsEnabled() || !controller->GetDrmDevice()->has_master()) {
      continue;
    }

    DrmWindow* window = FindWindowAt(controller_bounds);
    if (!window) {
      VLOG(1) << "    no exact bounds match for controller bounds="
              << controller_bounds.ToString();
      continue;
    }

    VLOG(1) << "    mapped window bounds=" << window->bounds().ToString()
            << " to controller_bounds=" << controller_bounds.ToString();
  ...
  for (auto& pair : window_map_) {
    ...
    VLOG(1) << "  apply mapping widget=" << pair.first
            << " window_bounds=" << pair.second->bounds().ToString()
            << " controller_bounds="
            << (controller
