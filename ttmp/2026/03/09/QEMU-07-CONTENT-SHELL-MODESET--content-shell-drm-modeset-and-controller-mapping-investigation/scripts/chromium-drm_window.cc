...
void DrmWindow::SetBounds(const gfx::Rect& bounds) {
  TRACE_EVENT2("drm", "DrmWindow::SetBounds", "widget", widget_, "bounds",
               bounds.ToString());
  VLOG(1) << "DrmWindow::SetBounds widget=" << widget_ << " old_bounds="
          << bounds_.ToString() << " new_bounds=" << bounds.ToString();
...
void DrmWindow::SchedulePageFlip(
  ...
  if (!controller_ || !controller_->GetDrmDevice()->has_master()) {
    VLOG(1) << "DrmWindow::SchedulePageFlip widget=" << widget_
            << " controller="
            << (controller_ ? "present-no-master" : "null")
            << " bounds=" << bounds_.ToString()
            << " plane_count=" << planes.size()
            << " -> ack without real flip";
    std::move(submission_callback)
...
void DrmWindow::SetController(HardwareDisplayController* controller) {
  ...
  const std::string controller_desc =
      controller_
          ? gfx::Rect(controller_->origin(), controller_->GetModeSize())
                .ToString()
          : std::string("(null)");
  VLOG(1) << "DrmWindow::SetController widget=" << widget_
          << " bounds=" << bounds_.ToString()
          << " controller_bounds=" << controller_desc;
