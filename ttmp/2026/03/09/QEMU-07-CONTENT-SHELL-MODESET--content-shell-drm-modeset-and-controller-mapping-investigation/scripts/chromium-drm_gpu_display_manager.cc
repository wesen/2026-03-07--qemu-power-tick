...
MovableDisplaySnapshots DrmGpuDisplayManager::GetDisplays() {
  ...
  VLOG(1) << "DrmGpuDisplayManager::GetDisplays begin";
  ...
  for (const auto& drm : devices) {
    ...
    VLOG(1) << "DrmGpuDisplayManager::GetDisplays drm="
            << drm->device_path().value()
            << " raw_display_info_count=" << display_infos.size();
    ...
    VLOG(1) << "DrmGpuDisplayManager::GetDisplays filtered_display_info_count="
            << display_infos.size();
...
void DrmGpuDisplayManager::NotifyScreenManager(
  ...
  VLOG(1) << "DrmGpuDisplayManager::NotifyScreenManager new_displays="
          << new_displays.size() << " old_displays=" << old_displays.size();
  ...
  for (const auto& new_display : new_displays) {
    if (std::ranges::none_of(old_displays,
      ...
      VLOG(1) << "DrmGpuDisplayManager::NotifyScreenManager add display_id="
