...
HIDE_TOOLBAR=${CONTENT_SHELL_HIDE_TOOLBAR:-0}
...
echo "[content-shell-drm-launcher] env EGL_PLATFORM=${EGL_PLATFORM:-<unset>} LD_LIBRARY_PATH=$LD_LIBRARY_PATH CONTENT_SHELL_VERBOSE=${CONTENT_SHELL_VERBOSE:-<unset>} CONTENT_SHELL_VMODULE=${CONTENT_SHELL_VMODULE:-<unset>} WINDOW_SIZE=$WINDOW_SIZE FULLSCREEN=$FULLSCREEN HIDE_TOOLBAR=$HIDE_TOOLBAR" >&2
...
if [ "$HIDE_TOOLBAR" = "1" ]; then
  set -- "$@" --content-shell-hide-toolbar
fi
