...
void render(struct app *app) {
    ...
    snprintf(line, sizeof(line), "SUSPENDS: %" PRIu64, app->suspend_cycles);
    ...
    draw_text(app, 48, 490, line, 2, fg);

    snprintf(line, sizeof(line), "LAST SLEEP MS: %" PRIu64, app->last_cycle.sleep_ms);
    draw_text(app, 48, 520, line, 2, fg);

    snprintf(line, sizeof(line), "LAST REDRAW MS: %" PRIu64, app->last_cycle.redraw_latency_ms);
    draw_text(app, 48, 550, line, 2, fg);

    snprintf(line, sizeof(line), "LAST RECONNECT MS: %" PRIu64, app->last_cycle.reconnect_latency_ms);
    draw_text(app, 48, 580, line, 2, fg);

    snprintf(line, sizeof(line), "PM TEST: %s", app->pm_test);
    draw_text(app, 48, 610, line, 2, fg);
    ...
    if (!app->last_cycle.redraw_recorded && app->last_cycle.resume_boot_ns != 0) {
        app->last_cycle.redraw_latency_ms = ns_to_ms(boot_ns() - app->last_cycle.resume_boot_ns);
        app->last_cycle.redraw_recorded = true;
        log_metric_pair(app, "resume_to_redraw", app->last_cycle.redraw_latency_ms);
    }
}
