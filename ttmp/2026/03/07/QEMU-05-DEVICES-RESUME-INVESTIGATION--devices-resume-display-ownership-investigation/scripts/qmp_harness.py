...
def parse_args() -> argparse.Namespace:
    ...
    subparsers.add_parser("query-commands")
    subparsers.add_parser("query-qmp-schema")

    screendump = subparsers.add_parser("screendump")
    screendump.add_argument("--file", required=True)
    screendump.add_argument("--device")
    screendump.add_argument("--head", type=int)
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "query-status":
            ...
            response = client.execute(args.name, json.loads(args.args_json))
        else:
