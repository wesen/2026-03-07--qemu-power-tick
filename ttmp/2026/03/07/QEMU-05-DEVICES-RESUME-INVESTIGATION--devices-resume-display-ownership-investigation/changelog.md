...
- Added a QMP/HMP state snapshot harness and showed that `info pci` is unchanged across resume, `info qtree` only changes USB enumeration order, and `x-query-virtio-status` for the virtio-gpu backend only resets `isr` and `queue-sel` on both device models, which does not explain the different bad capture shapes.
