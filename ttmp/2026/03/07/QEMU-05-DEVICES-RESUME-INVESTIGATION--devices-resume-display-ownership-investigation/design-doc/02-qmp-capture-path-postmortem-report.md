...
RelatedFiles:
    ...
    - Path: guest/kms_pattern.c
      Note: Bare-KMS helper that now reproduces the post-resume capture divergence without Weston or Chromium
    ...
    - Path: host/probe_screendump_support.py
      Note: QMP schema probe used to confirm explicit `screendump` device/head support
...
This report captures the current state of the `pm_test=devices` resume investigation after the stage-2 and stage-3 lab work converged on one narrower question: why does host-side QMP `screendump` diverge from guest-visible display state after resume?
...
- on `virtio-vga`, host-side QMP `screendump` in the same runs still falls to a `720x400` firmware-looking image,
- on `virtio-gpu-pci`, the post-resume `screendump` no longer falls to `720x400`, but it is still an almost-black `1280x800` frame,
- in the corrected bare-KMS control, guest DRM state points at a `kms_pattern`-owned `1280x800` framebuffer after resume while QMP still returns a `720x400` frame.
...
4. Shift the next investigation step toward QEMU `screendump` / display-head selection, not more guest-side repaint guessing.
...
### Decision: Treat `virtio-gpu-pci` as a diagnostic control, not a complete fix

Reason:
- switching away from `virtio-vga` removes the `720x400` text-looking fallback,
- but the default post-resume screenshot is still wrong and nearly black,
- so the display-device swap narrows the bug but does not close it.

Result:
- the working theory is now “default QEMU capture source is wrong after resume,” not merely “legacy text mode came back.”

### Decision: Use the corrected bare-KMS control as the strongest witness

Reason:
- the corrected `kms_pattern` run removes Weston and Chromium,
- programs a new KMS-owned framebuffer after resume,
- and still produces the wrong host-visible QMP result on `virtio-vga`.

Result:
- future explanations that depend on compositor or browser behavior are much weaker than host/QEMU explanations.
...
- and the corrected bare-KMS control now reproduces the divergence below Weston and Chromium entirely.
...
Investigate QEMU `screendump` target selection now that guest-side truth sources remain healthy after resume and the active QMP schema confirms `device/head` support.
...
3. Add one host/QEMU-side differentiator per run:
   ...
   - different QMP screen/head selection now that it is confirmed available,
   - host query of display surfaces or consoles if QEMU exposes them,
   - simpler guest KMS test if needed.
4. Compare whether the host-visible capture continues to show the wrong plane while guest KMS remains on the expected `1280x800` plane.
...
If guest KMS and guest screenshots agree while QMP disagrees, the host capture path is the broken layer. The corrected `kms_pattern` control now satisfies that condition directly.
...
- Why does `virtio-gpu-pci` avoid the `720x400` fallback but still return an almost-black `1280x800` screenshot after resume?
- Will explicit `screendump --device/--head` selection recover the correct scanout once the QEMU display devices have stable ids?
